...
vector=np.array([2**(k-1) for k in range(n)])  # [2**0,2**1,2**2,...,2**(k-1)]   vector of size n
...
for num in range (0,nbframe): #create patchwork for each iteration of GoL
    ...
    for k in range(n):
        ...
        #this for loop can be replaced by sum=np.dot(np.array(list(grille_dic.items()))[:,1].T,vector) or taking either the floor or ceil of this dot product
